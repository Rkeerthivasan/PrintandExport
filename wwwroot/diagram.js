...
window.updateAllowDragValue = async function (element,  allowDrag) {
    for (var i = symbolPaletteInstance.length - 1; i >= 0; i--) {
        if (symbolPaletteInstance[i].id == element.children[0].id) {
            symbolPaletteInstance[i].allowDrag = allowDrag;
        }
    }
}
window.initialiseModule = async function (element, component,allowDrag) {
    
    var symbolPaletteInstance = 'symbolPaletteInstance';
    if (window[symbolPaletteInstance]) {
        var object = { id: element.children[0].id, componentInstance: component, allowDrag: allowDrag }
    ...
    element.addEventListener('mouseup', _.throttle(e =>
        invokePaletteEvents(e, component), 0));
    setTimeout(function () {
            this.symbolPaletteDragAndDropModule = new InitDraggable(element, window[symbolPaletteInstance], allowDrag);
    }, 1000);
}
...
var InitDraggable =
    ...
    function () {
         ...
         * @hidden
         */
        function InitDraggable(parent, symbolPaletteInstance,allowDrop) {
            var _this = this;
            this.over = function (e) {
                const previewElementValue = "clonedNode";
            ...
            this.drag = function (e) {
                ...
                if (!parentsUntil(e.target, diagramClass)) {
                    ...
                    var canAllowDrag;
                    for (var i = symbolPaletteInstance.length - 1; i >= 0; i--) {
                        if (e.element.id === symbolPaletteInstance[i].id) {
                            canAllowDrag = symbolPaletteInstance[i].allowDrag
                            break;
                        }
                    }
                    if (previewElement) {
...
function invokeDiagramEvents(e, component) {
    ...
    if ((e.type == "mousemove" && !window.eventStarted) || e.type != "mousemove" || !window.inAction) {
        ...
        console.log(e.type, ",", args.clientX, ",", args.clientY, ",", args.detail, ",", args.offsetX, ",", args.offsetY, ",", args.screenX, ",", args.screenY);
    }
    if (e.type == "mouseup") {
